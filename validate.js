...
router.get('/', async (req, res) => {
  try {
    ...
    if (!sessionId || !sessionId.startsWith('SANCHO-XMD;;;')) {
      return res.status(400).json({
        valid: false,
        message: "Invalid session ID format. It should start with 'SANCHO-XMD;;;'."
      });
    }

    // Extract the file identifier
    const fileId = sessionId.split('SANCHO-XMD;;;')[1];
    if (!fileId) {
      return res.status(400).json({
        valid: false,
